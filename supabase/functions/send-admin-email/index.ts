...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (event === 'report') {
      ...
      const reporterName = reporterRes.data?.username ?? reporterId;
      const reportedName = reportedRes.data?.username ?? reportedUserId ?? 'unknown';

      subject = `[Doji] Content Report — ${reason}`;
      html = `
        <h2 style="margin:0 0 16px">Content Report Received</h2>
        <table style="border-collapse:collapse;font-family:sans-serif;font-size:14px">
          <tr><td style="padding:4px 12px 4px 0;color:#666">Reason</td><td><strong>${reason}</strong></td></tr>
          ...
          ${postId ? `<tr><td style="padding:4px 12px 4px 0;color:#666">Post ID</td><td>${postId}</td></tr>` : ''}
          <tr><td style="padding:4px 12px 4px 0;color:#666">Report ID</td><td>${reportId}</td></tr>
        </table>
      ...
      const blockerId = payload.blocker_id as string;
      const blockedId = payload.blocked_id as string;

      const [blockerRes, blockedRes] = await Promise.all([
        supabase.from('profiles').select('username').eq('id', blockerId).maybeSingle(),
        supabase.from('profiles').select('username').eq('id', blockedId).maybeSingle(),
      ]);

      const blockerName = blockerRes.data?.username ?? blockerId;
      const blockedName = blockedRes.data?.username ?? blockedId;

      subject = `[Doji] User Block — @${blockerName} blocked @${blockedName}`;
      html = `
        <h2 style="margin:0 0 16px">User Block</h2>
        <table style="border-collapse:collapse;font-family:sans-serif;font-size:14px">
          <tr><td style="padding:4px 12px 4px 0;color:#666">Blocker</td><td>@${blockerName}</td></tr>
          <tr><td style="padding:4px 12px 4px 0;color:#666">Blocked</td><td>@${blockedName}</td></tr>
        </table>
        <p style="margin-top:24px;font-family:sans-serif;font-size:14px;color:#666">
          Informational only. The blocked user's content has been removed from the blocker's feed.
          Review if this block appears to be triggered by abuse.
        </p>
      `;
    } else {
