-- A friendship grants visibility to social activity prospectively. Historical
-- completions, community reactions, and community comments from before the
-- relationship was accepted must not appear as new Activity Center items.
...
begin
  ...
  with friends as (
    select
      case when f.requester_id = uid then f.addressee_id else f.requester_id end user_id,
      f.accepted_at
      ...
      and (
        ...
        or (
          ...
          and exists (
            ...
            where friend.user_id = r.user_id and r.created_at >= friend.accepted_at
          )
        )
      )
  ), participant_rows as (
    ...
    join friends friend on friend.user_id = event.user_id
    ...
    select c.*, post.user_id post_owner_id, coalesce(post.is_community_poll, false) community,
      ...
      exists (
        select 1 from friends friend
        where friend.user_id = c.user_id and c.created_at >= friend.accepted_at
      ) friendship_existed,
      jsonb_build_object(
        'username', actor.username, 'display_name', actor.display_name,
        'avatar_url', actor.avatar_url, 'equipped_border_key', actor.equipped_border_key
...
revoke all on function public.get_notification_center_snapshot_without_post_context(timestamptz, integer)
  from public, anon;
grant execute on function public.get_notification_center_snapshot_without_post_context(timestamptz, integer)
  to authenticated;

comment on function public.get_notification_center_snapshot_without_post_context(timestamptz, integer) is
  'Bounded Activity Center snapshot with friend-scoped social activity filtered at friendship acceptance time.';
