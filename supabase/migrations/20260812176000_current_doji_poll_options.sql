-- The participation screen must not wait for a second request after learning
-- which challenge is live. Poll choices are bounded authoring data, so return
-- them with the authoritative current-state snapshot.
