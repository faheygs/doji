-- The realtime-token Edge Function binds the Ably clientId to the authenticated
-- Supabase user. Keep that identity in the one-round-trip capability contract.
-- This corrective migration is separate because the preceding migration may
-- already be applied in production and migrations are immutable history.
...
comment on function public.get_realtime_token_capabilities(uuid[]) is
  'Returns authenticated identity and bounded channel capabilities for one Ably token request.';
