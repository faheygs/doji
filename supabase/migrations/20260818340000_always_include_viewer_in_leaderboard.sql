-- Keep leaderboard reads bounded while guaranteeing that the signed-in viewer
-- can always find their own standing. The top result window remains shared;
-- when the viewer falls outside it, their authoritative row is appended.
...
begin
  ...
  with visible_profiles as materialized (
    ...
    where
      -- The viewer is always part of their own board. Demo/banned accounts remain
      -- excluded from every other user's competitive results.
      (profile.id = uid or (
        coalesce(profile.is_banned, false) = false
...
comment on function public.get_leaderboard_snapshot(text, text, integer) is
  'Bounded public standings: top rows plus the signed-in viewer, with friends mode restricted to accepted friends plus self.';
