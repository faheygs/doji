-- Blocking is a social-safety boundary, not a competition filter. The global
-- board remains neutral; the friends board naturally follows accepted friendships.
create or replace function public.get_leaderboard_snapshot(
  p_mode text default 'weekly', p_audience text default 'everyone',
...
begin
  ...
  with visible_profiles as (
    ...
    where coalesce(profile.is_banned, false) = false
      ...
      and (p_audience = 'everyone' or profile.id = uid or exists (
    ...
    select profile.id user_id,
      case when p_mode = 'weekly' then coalesce(weekly.xp, 0)
        else coalesce(profile.xp, 0) end::integer xp, profile
    from visible_profiles profile left join public.weekly_xp weekly
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'rank', ranked_position, 'user_id', user_id, 'xp', xp,
    'profile', jsonb_build_object(
      'id', (profile).id, 'username', (profile).username,
      'display_name', (profile).display_name, 'avatar_url', (profile).avatar_url,
      'avatar_gradient', (profile).avatar_gradient, 'bio', (profile).bio,
      'current_streak', (profile).current_streak,
      'longest_streak', (profile).longest_streak,
      'total_completions', (profile).total_completions,
      'total_missed', (profile).total_missed, 'xp', (profile).xp,
      'level', (profile).level, 'reactions_received', (profile).reactions_received,
      ...
      'is_admin', (profile).is_admin, 'is_banned', (profile).is_banned,
      'is_demo_account', (profile).is_demo_account,
      'created_at', (profile).created_at, 'updated_at', (profile).updated_at
    )) order by ranked_position), '[]'::jsonb) into result
  from (select row_number() over (order by xp desc, user_id)::integer ranked_position, *
    from ranked) ordered;
  return result;
end;
$$;
